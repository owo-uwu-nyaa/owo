...
def get_key(user,default,*path):
    if user==None and default==None:
        raise MissingKeyException(*path)
    elif len(path)==0:
        return user if user!=None else default
    else:
        key=path[1]
        try:
            return get_key(
                user[key] if user!=None and key in user else None,
                default[key] if default!=None and key in default else None,
                *(path[1:])
            )
