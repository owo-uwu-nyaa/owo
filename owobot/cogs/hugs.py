...
class Hugs(commands.Cog):
    ...
    async def h(self, ctx, shorthand: str, member: discord.Member):
        tags = " ".join(map(lambda x: x.val, HugShort.select(HugShort.val).where(HugShort.key.in_(list(shorthand)))))
