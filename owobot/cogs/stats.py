    ...
    dfw = (
        df.select(
            F.explode(
                F.split(
                    F.regexp_replace(
                        F.regexp_replace(F.lower(F.col("msg")), "[^a-z $]", ""),
                        "[ ]+",
                        " ",
                    ),
                    " ",
...
class Stats(commands.Cog):
    ...
    def get_messages_by_author(self, ctx):
        return self.df_global.filter(
            (F.col("author_id") == ctx.author.id) & (F.col("guild_id") == ctx.guild.id)
        )
    ...
    async def get_id_name_df(self, ctx):
        ...
        for row in dfq.collect():
            try:
                name = (
                    await common.author_id_to_obj(self.bot, row["author_id"], ctx)
                ).display_name
    ...
    async def emotes(self, ctx):
        with self.spark_lock:
            ...
            dfw = (
                dfa.select(
                    F.explode(
                        F.expr("regexp_extract_all(msg, '<a?:[^:<>@*~]+:\\\\d+>', 0)")
                    ).alias("emote")
                )
                .groupBy("emote")
                .count()
            ...
            pandas_df = dfw.toPandas().sort_values(by=["count"], ascending=False)
        ...
        with self.spark_lock:
            ...
            if len(members) == 0:
                ...
                for row in dfq.collect():
                    try:
                        name = (
                            await common.author_id_to_obj(
                                self.bot, row["author_id"], ctx
                            )
                        ).display_name
                        author_mappings.append((name, row["author_id"]))
                    except:
                        author_mappings.append(
                            (str(row["author_id"]), row["author_id"])
                        )
            else:
                for member in members:
                    author_mappings.append((common.get_nick_or_name(member), member.id))
            author_mappings_df = spark.createDataFrame(
                data=author_mappings, schema=["name", "id"]
            )
            # get msg counts
            dfg = self.get_guild_df(ctx).select("author_id", "time")
            dft = (
                dfg.join(
                    author_mappings_df.drop("name"),
                    dfg["author_id"] == author_mappings_df["id"],
                )
    ...
    async def couples(self, ctx):
        with self.spark_lock:
            ...
            df_hug_counts = (
                df_hugs.filter(df_hugs["hug_target"].isNotNull())
            ...
            df_hug_counts_names = (
                df_hug_counts.join(
                    author_mappings, df_hug_counts["_1"] == author_mappings["id"]
                )
                .withColumnRenamed("name", "#1")
                .drop("id", "_1")
                .join(author_mappings, df_hug_counts["_2"] == author_mappings["id"])
                .withColumnRenamed("name", "#2")
                .drop("id", "_2")
                .orderBy("count", ascending=False)
                .select("#1", "#2", "count")
            )
