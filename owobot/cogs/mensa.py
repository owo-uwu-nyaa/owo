...
def mensa_embed_builder(mensa, date):
    ...
    """
    embed = discord.Embed(title=f'{date.strftime("%A, %-d %b %Y")}', color=0x3EDB53)
...
class Mensa(commands.Cog):
    ...
    async def info(self, ctx, mensa_name="garching"):
        ...
        if not mensa:
            return await ctx.send(
                "Please select one of the following canteens: "
                + ", ".join(mensa_api.MENSA_LIST.keys())
            )
        ...
        if occupation:
            ...
            embed.description = (
                f'[ {int(occupation)} / {mensa.get("capacity", 0)} :people_hugging: ] '
                + ((bar + f"({round(percent*100,2):1.2f}%)") if capacity else "")
            )
        ...
        for dish_type, label in labels.items():
            if dish_type != "Beilagen":
                ...
                embed.add_field(
                    name=dish_type + " " + mensa_api.TYPES.get(dish_type, ""),
                    value=value,
                    inline=True,
                )
                continue
            embed.add_field(
                name="More",
                value=f"Other {len(label)} side-dishes can be seen with the command `{self.bot.command_prefix}mensa menu`",
                inline=True,
            )
    ...
    async def menu(self, ctx, mensa_name="garching", page_number=1, page_size=6):
        ...
        embed.description = (
            f'Menu for the {mensa.get("display_name", "")} mensa. Provided by the [eat-api](https://tum-dev.github.io/eat-api/).'
            + (
                f" Notice: this menu is for _{date.strftime('%A')}_!"
        ...
        for dish in (
            dishes_today[index : index + page_size] if pages > 1 else dishes_today
        ):
            embed.add_field(
                name=dish.get("name", "?") + " " + dish.get("emoji", ""),
                value=f'`{dish["dish_type"]}` €{dish["prices"]["students"]["price_per_unit"]} / {dish["prices"]["students"]["unit"]}\n\n{" ".join(map(lambda x : f"`{mensa_api.LABELS.get(x)}`", dish["labels"]))}',
                inline=True,
            )
    ...
    async def labels(self, ctx):
        ...
        await ctx.send(
            "\n".join(
                sorted(
                    [f"`{i}`: {v}" for i, v in mensa_api.LABEL_MAP.items()],
                    reverse=True,
                )
            )
        )
    ...
    async def mensaplot(self, ctx, dayofweek: int = -1):
        ...
        df = pd.read_csv(
            self.bot.config.mensa_csv,
            names=["time", "count"],
            dtype={"time": float, "count": int},
        )
        df["time"] = (
    ...
    async def update_channel(self):
        ...
        for channel in channels:
            await channel.edit(
                name=f'{int(occupation)} / {int(mensa.get("capacity", 0))} {(mensa["display_name"]).capitalize()}'
                + random.choice(list(mensa_api.TYPES.values()))
            )
